...
#   2018.08.24  Jessica Zaiss-Bowman, leading zeros on dates; Python3; datetime



#################################
# user parameters to be edited: #
#################################
...
def yearday(day,month,year):
  aa=str(month)+'/'+str(day)+'/'+str(year)
  import datetime
  yday = datetime.datetime.strptime(aa,'%m/%d/%Y').timetuple().tm_yday
  return(yday)

  
def span(i1,i2,i3=1):
   return range(int(i1),int(i2+i3/abs(i3)),int(i3))
...
def calday(yday,year):
    aa=str(year)+'/'+str(yday)
    import datetime
    xx=datetime.datetime.strptime(aa,'%Y/%j')
    month = xx.month
    day = xx.day
    return(day,month)
...
if options.date1==-1:
    ...
    date1 = options.date1
year0=int( date0[0] ); month0=int( date0[1] ); day0=int( date0[2] );
year1=int( date1[0] ); month1=int( date1[1] ); day1=int( date1[2] );
...
try:  # override input() for Python 2.x and older:
    input = raw_input
except NameError:  # you're running Python 3.x:
    pass
